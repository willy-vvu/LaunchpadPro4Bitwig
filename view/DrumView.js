...
DrumView.prototype.onGridNote = function (note, velocity)
{
    ...
    if (x < 4)
    {
        ...
        if (this.surface.isDeletePressed ())
        {
            ...
            this.surface.setButtonConsumed (LAUNCHPAD_BUTTON_DELETE);
            this.clip.clearRow (this.offsetY + this.selectedPad);
        }
        else if (this.surface.isPressed (LAUNCHPAD_BUTTON_MUTE))
        {
            // Mute that 'pad'
            this.model.getTrackBank ().primaryDevice.toggleLayerOrDrumPadMute (playedPad);
        }
        else if (this.surface.isPressed (LAUNCHPAD_BUTTON_SOLO))
        {
            // Solo that 'pad'
...
DrumView.prototype.drawGrid = function ()
{
    ...
    for (var pad = 0; pad < 16; pad++)
        this.surface.pads.lightEx (4 + pad % 4, 4 + Math.floor (pad / 4), pad >= loopStartPad && pad < loopEndPad ? (pad == currentMeasure ? LAUNCHPAD_COLOR_GREEN : LAUNCHPAD_COLOR_WHITE) : LAUNCHPAD_COLOR_BLACK, null, false);
...
DrumView.prototype.getPadColor = function (index, primary, hasDrumPads, isSoloed, isRecording)
{
    ...
    if (this.pressedKeys[this.offsetY + index] > 0)
        return isRecording ? LAUNCHPAD_COLOR_RED_HI : LAUNCHPAD_COLOR_GREEN_HI;
    // Selected?
    if (this.selectedPad == index)
        return LAUNCHPAD_COLOR_BLUE_HI;
    // Exists and active?
    var drumPad = primary.getDrumPad (index);
    if (!drumPad.exists || !drumPad.activated)
        return LAUNCHPAD_COLOR_YELLOW_LO;
    // Muted or soloed?
    if (drumPad.mute || (isSoloed && !drumPad.solo))
        return LAUNCHPAD_COLOR_AMBER_LO;
    return drumPad.color ? drumPad.color : LAUNCHPAD_COLOR_YELLOW_HI;
};
